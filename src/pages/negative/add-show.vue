<template>
  <div style="background:#fff;">
    ...
    <div
      class="content-main"
      style="height: calc(100% - 100px);padding:20px 60px 60px 20px;"
    >
      <a-form-model
        :model="thisForm"
        layout="inline"
      ...
      >
        <div class="common-title">
          <div class="common-title-content">
            {{ $route.path === '/negative/add' ? '新增' : '查看' }}负数单
          </div>
        </div>
        <a-form-model-item
          label="选择销售单"
        ...
        <a-form-model-item
          label="销售单号"
          v-if="$route.path === '/negative/show'"
          >{{ saleOrderNo }}</a-form-model-item
        >
        <a-form-model-item
          label="负数单号"
          v-if="$route.path === '/negative/show'"
          >{{ $route.query.negativeNo }}</a-form-model-item
        >
        <a-form-model-item
          label="创建人"
          v-if="$route.path === '/negative/show'"
          >{{ createUserName }}</a-form-model-item
        >
        <a-form-model-item
          label="创建时间"
          v-if="$route.path === '/negative/show'"
          >{{ createTime }}</a-form-model-item
        >
        <div class="common-title">
        ...
        <a-table
          ...
          :rowKey="(r, i) => i"
          :scroll="{ x: 2500, y: 400 }"
          v-else-if="$route.path === '/negative/show'"
        >
          <template slot="itemSpecs" slot-scope="scope">
            <span v-html="itemSpecsParse(scope.itemSpecs)"></span>
          </template>
        ...
        <div style="display: flex">
          <div style="flex: 1">
            <a-form-model-item label="税前销售总价">{{
              totalPretaxItemPrice
            }}</a-form-model-item>
            <a-form-model-item label="税后销售总价">{{
              totalAmount
            }}</a-form-model-item>
            <a-form-model-item label="税前优惠总价">{{
              totalPretaxReducedPrice
            }}</a-form-model-item>
            <a-form-model-item label="税后优惠总价">{{
              totalReducedPrice
            }}</a-form-model-item>
          </div>
          <div style="flex: 1">
            <a-form-model-item
              label="已扣税前销售总价"
              v-if="$route.path === '/negative/add'"
              >{{ pretaxItemPriceDeduct }}</a-form-model-item
            >
            <a-form-model-item
              label="已扣税后销售总价"
              v-if="$route.path === '/negative/add'"
              >{{ itemPriceDeduct }}</a-form-model-item
            >
            <a-form-model-item
              label="已扣税前优惠总价"
              v-if="$route.path === '/negative/add'"
              >{{ pretaxReducedPriceDeduct }}</a-form-model-item
            >
            <a-form-model-item
              label="已扣税后优惠总价"
              v-if="$route.path === '/negative/add'"
              >{{ reducedPriceDeduct }}</a-form-model-item
            >
            <a-form-model-item
              label="已扣税前销售总价"
              v-if="$route.path === '/negative/show'"
              >{{ totalPretaxItemPriceDeduct }}</a-form-model-item
            >
            <a-form-model-item
              label="已扣税后销售总价"
              v-if="$route.path === '/negative/show'"
              >{{ totalAmountDeduct }}</a-form-model-item
            >
            <a-form-model-item
              label="已扣税前优惠总价"
              v-if="$route.path === '/negative/show'"
              >{{ totalPretaxReducedPriceDeduct }}</a-form-model-item
            >
            <a-form-model-item
              label="已扣税后优惠总价"
              v-if="$route.path === '/negative/show'"
              >{{ totalReducedPriceDeduct }}</a-form-model-item
            >
          </div>
        </div>
        <a-form-model-item
        ...
        >
          <a-input
            ...
            @blur="
              totalReducedPriceDeduct = amountFormat(totalReducedPriceDeduct)
            "
            :disabled="!disInput"
          />
        </a-form-model-item>
    ...
    >
      ...
      <template slot="footer">
        <a-button type="primary" @click="showErrorTable = false"
          >我知道了，返回处理</a-button
        >
      </template>
    </a-modal>
    <FormSubmitButton
      :isShow="true"
      :isShowSubmit="$route.path === '/negative/add'"
      @submit="add()"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
        ...
        if (valid) {
          ...
          this.beSelected.forEach((item, index) => {
            let set1_limit = item.itemNum - item.itemNumDeduct,
              ...
              set4_limit =
                item.pretaxReducedPrice - item.pretaxReducedPriceDeduct,
          ...
          let total1_limit =
              this.totalPretaxItemPrice - this.pretaxItemPriceDeduct,
            total2_limit = this.totalAmount - this.itemPriceDeduct,
            total3_limit =
              this.totalPretaxReducedPrice - this.pretaxReducedPriceDeduct,
    ...
    calc() {
      ...
      this.totalPretaxReducedPriceDeduct = totalPretaxReducedPriceDeduct.toFixed(
        2
      )
    ...
    handleChange(value) {
      ...
      api
        .getNegativeItems({ orderNo: this.thisForm.saleOrderNo })
        .then(resp => {
          if (resp.code === 200) {
  ...
  mounted() {
    if (this.$route.path === '/negative/show') {
      ...
      api
        .queryNegativeDetail({ negativeNo: this.$route.query.negativeNo })
        .then(resp => {
          if (resp.code === 200) {
            ...
            this.totalPretaxReducedPrice = resp.data.totalPretaxReducedPrice
            this.totalReducedPrice = resp.data.totalReducedPrice
            this.totalPretaxItemPriceDeduct =
              resp.data.totalPretaxItemPriceDeduct
            this.totalAmountDeduct = resp.data.totalAmountDeduct
            this.totalPretaxReducedPriceDeduct =
              resp.data.totalPretaxReducedPriceDeduct
